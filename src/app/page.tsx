...
export default function Home() {
  ...
  const nicheGreetings: Record<string, { title: string; subtitle: string }> = {
    ecommerce: { title: 'ShopBot - AI Customer Support', subtitle: 'Ask about orders, returns, shipping, or products.' },
    real_estate: { title: 'PropertyBot - AI Support Agent', subtitle: 'Ask about viewings, buying, renting, or valuations.' },
    hr: { title: 'HR Assistant - AI Support Agent', subtitle: 'Ask about leave, payroll, benefits, or onboarding.' },
    healthcare: { title: 'ClinicBot - AI Support Agent', subtitle: 'Ask about appointments, prescriptions, or services.' },
    legal: { title: 'LegalBot - AI Support Agent', subtitle: 'Ask about consultations, fees, or practice areas.' },
  }
  ...
  return (
    <main className="min-h-screen flex flex-col" style={{ background: '#05070f' }}>
      ...
      <div className="relative z-10 flex flex-col min-h-screen max-w-3xl mx-auto w-full px-4 py-6">
        ...
        <div className="mb-4">
          <NicheSelector selectedNiche={niche} onSelect={handleNicheChange} onPromptClick={handlePromptClick} />
        </div>

        <div className="flex-1 flex flex-col rounded-2xl border border-[#1a2438] bg-[#08090f] overflow-hidden" style={{ minHeight: '420px' }}>
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
            {messages.length === 0 && (
              <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
                <div className="w-14 h-14 rounded-2xl bg-[#0b0f1a] border border-[#1a2438] flex items-center justify-center text-2xl mb-4">AI</div>
                <p className="text-slate-300 font-medium mb-1">Ask anything</p>
                <p className="text-slate-500 text-sm max-w-xs">This agent answers questions from a real knowledge base - not generic GPT responses.</p>
              </div>
            )}
            {messages.map((m, i) => (
            ...
            {isLoading && messages[messages.length - 1]?.role === 'user' && (
              <div className="flex gap-3">
                <div className="w-8 h-8 rounded-full bg-[#0f1623] border border-[#1a2438] flex items-center justify-center text-xs text-blue-400 font-semibold flex-shrink-0">AI</div>
        ...
        <div className="mt-4 text-center">
          <p className="text-slate-600 text-xs">
            Built by{' '}
            <a href="https://zainsverse.de" target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-400 transition-colors">zainsverse.de</a>
            {' '} - Next.js - OpenAI - Pinecone - Vercel -{' '}
            <a href="https://github.com/Hafiz-Muhammad-Zain/ai-support-chatbot" target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-400 transition-colors">View on GitHub</a>
          </p>
        </div>
      </div>
    </main>
